...
const WhyUsSection = () => (
  <section id="why-us" className="bg-muted px-6 py-24 md:py-32">
    <div className="mx-auto max-w-4xl">
      <h2 className="mb-6 text-center text-3xl font-bold tracking-tight text-foreground md:text-4xl">
        Why Buyers Engage Us
      </h2>
      <p className="mx-auto mb-4 max-w-xl text-center text-sm leading-relaxed text-muted-foreground md:text-base">
      ...
      <div className="grid gap-8 md:grid-cols-3">
        {pillars.map((p) => (
          <div key={p.title} className="rounded-md border border-border bg-card p-8 shadow-sm">
            <p.icon className="mb-5 h-7 w-7 text-accent" />
            <h3 className="mb-3 font-['Playfair_Display',Georgia,serif] text-lg font-semibold text-card-foreground">
              {p.title}
            </h3>
            <p className="max-w-xs text-sm leading-relaxed text-muted-foreground">{p.desc}</p>
          </div>
        ))}
      </div>
    </div>
